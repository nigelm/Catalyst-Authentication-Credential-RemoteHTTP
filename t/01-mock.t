...
## This set of tests were originally in Catalyst::Authentication::Credential::HTTP::Proxy
## but have been lifted almost verbatim to test this module.
## Thanks to Marcus Ramberg & Tomas Doran for doing the hard work.

BEGIN {
    use_ok('Catalyst::Authentication::Credential::RemoteHTTP');
}
...
throws_ok {
    Catalyst::Authentication::Credential::RemoteHTTP->new({}, $mock_c, $mock_realm);
} qr/Attribute \(.+\) is required/, 'No config throws';

lives_ok { 
    Catalyst::Authentication::Credential::RemoteHTTP->new(
        {url => 'http://some.proxy:8080'},
        $mock_c, $mock_realm,
    );
} 'Normal (with url) ok';

## We do not have a type parameter - but maybe should add one
#throws_ok {
#    Catalyst::Authentication::Credential::RemoteHTTP->new(
#        {url => 'http://some.proxy:8080',
#        type => 'foobar'},
#        $mock_c, $mock_realm,
#    );
#} qr/Catalyst::Authentication::Credential::RemoteHTTP/, 'Asking for unknown type throws';
...
my $cred = Catalyst::Authentication::Credential::RemoteHTTP->new(
    {url => 'http://some.proxy:8080',
    ##type => 'basic'
    },
    $mock_c, $mock_realm,
...
my ($auth_ua, $auth_res, $auth_url);
{
    ...
    *Catalyst::Authentication::Credential::RemoteHTTP::User::get = sub { $auth_ua = shift; $auth_url = shift; $auth_res };
}
